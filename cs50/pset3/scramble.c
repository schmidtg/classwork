...
bool lookup(string word)
{
    ...
    while ( first <= last ) {
        ...
        // store value of comparison
        int compare = strcmp(dictionary.words[middle].letters, word);

        // check if words hasn't been used and matches
        if (dictionary.words[middle].found == false
         && compare == 0
        ) {
...
void scramble(void)
{
    // create temporary grid to store new layout
    char scramble_grid[DIMENSION][DIMENSION];
    for (int i = 0; i < DIMENSION; i++) {
        for (int j = 0; j < DIMENSION; j++) {
            scramble_grid[j][DIMENSION - 1 - i] = grid[i][j];
        }
    }
