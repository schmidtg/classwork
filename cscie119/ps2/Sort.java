...
public class Sort {
     ...
     * Takes an already sorted array and shifts the remaining elements
     * into the leftmost positions. Returns an int that specifies the
     * number of unique values in the array
     *
     * @param arr - incoming array of integers
     * @return - number of unique items in array
     */
    private static int removeDups(int[] arr) {

        // Create incrementer and counter to store unique values
        int j = 0;
        int unique_values = 0;
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            // Write the first element as it'll always be unique
            ...
            // Not equal means its a unique value
            if (arr[i] != arr[i - 1]) {
                arr[j] = arr[i];

                // Increment unique_values counter and j iterator
                unique_values++;
                j++;
            }
        }
    ...
    public static void main(String[] arr) { 
        ...
        /* Remove duplicates */
        int uniques = removeDups(copy);
        System.out.print("removed duplicates: " + uniques + " unique values in array: \t");
        printArray(copy);
    }
}
